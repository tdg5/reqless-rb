...
require 'spec_helper'
require 'reqless/middleware/metriks'
    ...
    module Metriks
      ...
      describe TimeJobsByClass do
        ...
        it 'tracks the time taken by the job, grouped by the class name' do
          ...
          job = Reqless::Job.build(double, JobABC, {})
          ...
          timer = ::Metriks.timer('reqless.job-times.JobABC')
      ...
      describe CountEvents do
        ...
        def create_job_and_perform(klass, job_result = :complete)
          job = Reqless::Job.build(double, klass, {})
        ...
        it 'increments an event counter when a particular job completes' do
          ...
          expect(::Metriks.counter('reqless.job-events.foo').count).to eq(1)
          expect(::Metriks.counter('reqless.job-events.bar').count).to eq(0)

          create_job_and_perform(Class2)

          expect(::Metriks.counter('reqless.job-events.foo').count).to eq(1)
          expect(::Metriks.counter('reqless.job-events.bar').count).to eq(1)
        end

        it 'does not increment the counter if the job fails' do
          create_job_and_perform(Class1, :failed)
          expect(::Metriks.counter('reqless.job-events.foo').count).to eq(0)
        end
